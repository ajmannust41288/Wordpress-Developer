 ...
 * @return void
 */
function ecommerce_wp_breadcrumb_trail( $args = array() ) {
	
	$breadcrumb = apply_filters( 'ecommerce_wp_breadcrumb_trail_object', null, $args );

	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new ecommerce_wp_breadcrumb_trail( $args );
 ...
 * @access public
 */
class ecommerce_wp_breadcrumb_trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `ecommerce_wp_breadcrumb_trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see ecommerce_wp_breadcrumb_trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see ecommerce_wp_breadcrumb_trail::set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'ecommerce_wp_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		$breadcrumb = apply_filters( 'ecommerce_wp_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'ecommerce-wp' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'ecommerce-wp' ),
			'home'                => esc_html__( 'Home',                                  'ecommerce-wp' ),
			'error_404'           => esc_html__( '404 Not Found',                         'ecommerce-wp' ),
			'archives'            => esc_html__( 'Archives',                              'ecommerce-wp' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;',   'ecommerce-wp' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'ecommerce-wp' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'ecommerce-wp' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'ecommerce-wp' ),
		...
		$this->labels = apply_filters( 'ecommerce_wp_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'ecommerce_wp_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'ecommerce_wp_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'ecommerce-wp' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'ecommerce-wp' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'ecommerce-wp' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'ecommerce-wp' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'ecommerce-wp' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'ecommerce-wp' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'ecommerce-wp' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'ecommerce-wp' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'ecommerce-wp' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'ecommerce-wp' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'ecommerce-wp' ) ) ) );
