...
class ecommerce_wp_post_marquee_widget extends WP_Widget {
...
function __construct() {
		...
		'ecommerce_wp_post_marquee_widget', 
		  
		// Widget name will appear in UI
		__('+ Post Marquee', 'ecommerce-wp'), 
		  
		// Widget description
		array( 'description' => __( 'Display Post Post Marquee.', 'ecommerce-wp' ), ) 
		);
}
...
public function widget( $args, $instance ) {
		...
		echo '<div class="tech-slideshow-container">';	
		echo '<div class="tech-slideshow">';
		echo '<div class="mover-1">';
		...
		while( $loop->have_posts() ) : $loop->the_post();
			...
			<span><img style="height: 40px;width: 40px;object-fit: cover;" src="<?php echo esc_url($url); ?>" /></span>
...
function ecommerce_wp_post_marquee_widget() {
		register_widget( 'ecommerce_wp_post_marquee_widget' );
}
add_action( 'widgets_init', 'ecommerce_wp_post_marquee_widget' );
